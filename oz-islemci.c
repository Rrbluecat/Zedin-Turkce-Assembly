...
int parse_kaynak(const char *dosya_adi, int *kod_uzunlugu) {
    ...
    while (fscanf(f, "%127s", m) != EOF) {
        ...
        if (m[0] == '@')  continue;
        if (strcmp(m, "DATA") == 0) {
            int hedef_adres; char metin[128];
        ...
        switch (op) {
            ...
            case KESME: {
                ...
                switch (c) {
                    ...
                    case 60: {
                        char dizin[128];
                        ram_string_al(ceb, dizin, sizeof(dizin));
                        DIR *d = opendir(dizin);
                        if (!d) { printf("[HATA] Dizin acilamadi!\n"); break; }
                        struct dirent *girdi;
                        while ((girdi = readdir(d)) != NULL) {
                            if (girdi->d_name[0] == '.') continue;
                            printf("%s\n", girdi->d_name);
                        }
                        closedir(d);
                        break;
                    ...
                    case 61: {
                        char dosya[128];
                        ram_string_al(ceb, dosya, sizeof(dosya));
                        ceb = (remove(dosya) == 0) ? 1 : 0;
                        break;
                    }
                    default:
